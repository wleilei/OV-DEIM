...
from dist_tools.dist_utils import init_dist_from_env, get_available_gpus
from optim_tools.ema import ModelEMA
from optim_tools.warmup import WarmupConstantCosine
from optim_tools.utils import get_optim_params, log_param_groups_to_swanlab, log_param_groups, count_parameters
...
def train(args):
    ...
    if args.train_datasets == "o365":
        ...
        train_dataset = get_og_ori_dataset(data_o365_root=args.data.data_o365_root, 
                                ...
                                pipeline_aug=train_pipeline_base,
                                pipeline_clean=train_pipeline_base,
                                o365_dir_text=args.data.o365_dir_text,
                                global_dir_text=args.data.global_dir_text,
                                prob_aug=args.data.prob_aug
                                )
        train_dataset_aug = get_og_ori_dataset(data_o365_root=args.data.data_o365_root, 
                                           ann_o365_file=args.data.ann_o365_file, 
                                data_gqa_root=args.data.data_gqa_root, ann_gqa_file=args.data.ann_gqa_file, 
                                data_flickr_root=args.data.data_flickr_root, ann_flickr_file=args.data.ann_flickr_file,
                                cache_dir=args.data.cache_file_og,
    ...
    synthetic_sampler = DistributedSampler(train_dataset_aug, num_replicas=world_size, rank=rank, shuffle=True, drop_last=True)
    synthetic_dataloader = DataLoader(train_dataset_aug, batch_size=args.batch_size, 
                                  prefetch_factor=4,                       
    ...
    if rank == 0:
        log_param_groups(param_groups,model.named_parameters())
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for i in tqdm(range(num_iters), desc=f"Epoch {epoch+1}/{epochs}", disable=rank != 0):
            ...
            with autocast(device_type='cuda'):
                ...
                if denoising_keys:
                    ...
                    loss_denoising = torch.tensor(0.0, device=device)
            ...
            skip_step = torch.tensor(0.0, device=device)
            if torch.isnan(loss) or torch.isinf(loss):
                skip_step = torch.tensor(1.0, device=device)
...
def main():
    ...
    parser.add_argument("--num_training_classes", type=int, default=150)
    ...
    config.data.num_training_classes = args.num_training_classes
    config.decoder.num_classes = args.num_training_classes
    config.criterion.num_classes = args.num_training_classes
    config.criterion.alpha = args.alpha
