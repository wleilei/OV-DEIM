...
class OptimizerConfig:
    patterns: List[Dict] = field(default_factory=lambda: [
	    {'params': r'^(?=.*text_adapter)(?!.*(?:norm|bn|bias)).*$', 'lr': 1e-5},
        {'params': r'^(?=.*text_adapter)(?=.*(?:norm|bn|bias)).*$', 'lr': 1e-5, 'weight_decay': 0.0},
	    {'params': r'^(?=.*backbone)(?=.*dinov3)(?!.*(?:norm|bn|bias|gamma)).*$', 'lr': 1e-5},
        {'params': r'^(?=.*backbone)(?=.*dinov3)(?=.*(?:norm|bn|bias|gamma)).*$', 'lr': 1e-5, 'weight_decay': 0.0},
        {'params': r'^(?=.*backbone)(?!.*dinov3)(?!.*(?:norm|bn|bias|gamma)).*$', 'lr': 2e-4},
        {'params': r'^(?=.*backbone)(?!.*dinov3)(?=.*(?:norm|bn|bias|gamma)).*$', 'lr': 2e-4, 'weight_decay': 0.0},
